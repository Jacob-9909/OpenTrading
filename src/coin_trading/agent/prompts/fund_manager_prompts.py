...
2) stop_loss MUST be ≥ 2.5× primary_atr away from entry; take_profit MUST be ≥ 5.0× primary_atr away from entry. primary_atr is the context field `primary_atr` (primary timeframe ATR only — do NOT use ATR values from multi_timeframe for SL/TP sizing). Minimum absolute distance: SL ≥ 0.8% of entry, TP ≥ 1.5% of entry. Note: trailing stop activates only after price moves ≥ 0.4% in favor of the position — initial SL must absorb noise. Trailing TP extends 1.0% past breach; initial TP is a trigger point, not the final exit.
...
- LONG: stop_loss < entry_price < take_profit; SL ≥ 2.5×primary_atr and ≥ 0.8% below entry; TP ≥ 5.0×primary_atr and ≥ 1.5% above entry.
- SHORT: take_profit < entry_price < stop_loss; SL ≥ 2.5×primary_atr and ≥ 0.8% above entry; TP ≥ 5.0×primary_atr and ≥ 1.5% below entry.
